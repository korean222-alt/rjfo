...
console.log("\n[9] 봉 집계");
{
  ...
  assert(monthly[0].volume === days.reduce((a, b) => a + b.volume, 0), "월봉 거래량 = 전체 합");
  assert(aggregateBars(days, "1d").length === days.length, "일봉은 개수가 그대로");

  // 집계해도 거래량 총합은 보존된다 (실데이터에 가까운 픽스처로 한 번 더)
  const noisy = noisyFixture(400);
  for (const tf of ["1w", "1M"] as const) {
    const agg = aggregateBars(noisy, tf);
    approx(
      agg.reduce((a, b) => a + b.volume, 0),
      noisy.reduce((a, b) => a + b.volume, 0),
      1e-6,
      `${tf}: 거래량 총합 보존`,
    );
    assert(
      agg.every((b, i) => i === 0 || agg[i - 1].date < b.date),
      `${tf}: 봉이 날짜 오름차순`,
    );
    assert(
      agg.reduce((a, b) => a + b.dayCount, 0) === noisy.length,
      `${tf}: 들어간 일봉 수 합 = 원본 개수`,
    );
  }
