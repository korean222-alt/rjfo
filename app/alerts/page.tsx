...
import { ALERT_SIGNALS, type SignalKey } from "@/lib/presets";
...
function labelOf(signal: SignalKey): string {
  return ALERT_SIGNALS.find((s) => s.key === signal)?.label ?? signal;
}
...
export default function AlertsPage() {
  ...
  // 결과 화면에서 "이 신호 알림 받기"로 넘어온 경우 입력을 미리 채워 준다.
  // (useSearchParams는 Suspense 경계를 요구해서, 여기서는 그냥 주소창을 읽는다.)
  useEffect(() => {
  ...
  const add = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/alerts", {
        ...
        body: JSON.stringify({ ticker: t, signal }),
      });
      const data = (await res.json()) as AlertsState & { error?: string };
      if (!res.ok) throw new Error(data.error ?? "등록에 실패했습니다.");
      setState(data);
      setTicker("");
      setNotice(`${t} · ${labelOf(signal)} 알림을 등록했습니다.`);
    } catch (e) {
    ...
    <main className="mx-auto max-w-lg px-4 py-8 pb-24">
      ...
      <header className="mb-6 mt-4">
        ...
        <p className="mt-1.5 text-sm text-muted leading-relaxed">
          등록한 종목에 신호가 뜨는 날, 미국장 마감 뒤에 텔레그램으로 알려드립니다.
        </p>
      </header>
        ...
        <section className="mb-6 rounded-2xl border border-amber-500/40 bg-amber-500/10 p-4 text-sm leading-relaxed">
          ...
          <ol className="mt-2 list-decimal space-y-1.5 pl-5 text-amber-100/90">
            {!state.telegram.botToken ? (
              <li>
                텔레그램에서 <span className="font-semibold">@BotFather</span>에게 <code>/newbot</code>을
                보내 봇을 만들고, 받은 토큰을 Vercel 환경변수{" "}
                <code className="text-amber-200">TELEGRAM_BOT_TOKEN</code>에 넣으세요.
              </li>
            ) : null}
            {!state.telegram.chatId ? (
              <li>
                방금 만든 봇에게 아무 메시지나 한 번 보낸 뒤{" "}
                <Link href="/api/alerts/chat-id" className="underline">
                  이 주소
                </Link>
                를 열어 나온 id를 <code className="text-amber-200">TELEGRAM_CHAT_ID</code>에 넣으세요.
              </li>
            ) : null}
            {!state.storage ? (
              <li>
                Vercel 프로젝트에 KV(Upstash Redis) 스토어를 연결하세요. 알림 목록을 저장할 곳입니다.
              </li>
            ) : null}
          </ol>
          <p className="mt-2 text-xs text-amber-100/70">
            환경변수를 바꾼 뒤에는 다시 배포해야 적용됩니다.
          </p>
        </section>
      ) : null}
      ...
      {telegramReady ? (
        <button
          ...
          onClick={sendTest}
          disabled={busy}
          className="mb-6 w-full rounded-xl border border-border bg-surface py-3 text-sm font-medium disabled:opacity-50"
        >
          테스트 알림 보내기
        </button>
      ) : null}
      ...
      <section className="space-y-4 rounded-2xl border border-border bg-surface p-4">
        ...
        <div>
              ...
              return (
                <button
                  ...
                  className={`rounded-full border px-3.5 py-2 text-sm transition active:scale-95 ${
                    on ? "border-blue-400 bg-blue-500/15 text-white" : "border-border bg-bg text-muted"
                  }`}
                >
                  {s.label}
                </button>
          ...
          <p className="mt-2 text-xs text-muted leading-relaxed">
            {ALERT_SIGNALS.find((s) => s.key === signal)?.hint}
          </p>
        </div>
      ...
      {error ? (
        <p className="mt-4 rounded-xl border border-down/40 bg-down/10 px-4 py-3 text-sm text-down">
          {error}
        </p>
      ) : null}
      {notice ? (
      ...
      <h2 className="mb-2 mt-8 text-sm font-semibold text-muted">
        등록된 알림 {state ? `(${state.watches.length})` : ""}
      </h2>
            ...
            >
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-xs text-muted">
                  {labelOf(w.signal)}
                  {w.lastNotifiedDate ? ` · 마지막 알림 ${w.lastNotifiedDate}` : ""}
                </p>
              </div>
              <button
                type="button"
                onClick={() => remove(w.id)}
                disabled={busy}
                className="shrink-0 rounded-lg border border-border bg-bg px-3 py-2 text-sm text-muted disabled:opacity-50"
              >
                삭제
              </button>
            </li>
