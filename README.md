...
모델 하나에 주는 시간은 **남은 예산에 따라 늘어난다**(최소 5.5초, 최대 14초, 다음 후보용 2초는
남긴다). 예전에는 5.5초 고정이었는데, 그게 실제로 "주식은 되는데 코인만 AI가 안 뜨는" 원인이었다.
`/api/diag?ai=1`로 재보니 **gemini-3.8-flash가 32토큰짜리 "연결됨" 한마디에 5.8초**(503 재시도가
끼면 9.6초)를 쓴다. 5.5초 상한 아래서는 후보를 아무리 늘려도 전부 같은 자리에서 잘리고,
프롬프트가 조금이라도 더 긴 종목(코인은 펀딩 지표가 붙어 리포트가 길다)부터 먼저 넘어간다.

서버 계산은 병목이 아니다 — 5,040봉 · 지표 31개 채점 · 조합 23개가 **0.5초**다(`enrich` 41ms +
`analyzeCycle` 452ms). 느린 건 언제나 모델 쪽이고, 그래서 요약을 기다리는 구조 자체를 없앴다
(바로 아래).

**AI 문장은 화면을 막지 않는다.** `/api/cycle`·`/api/candle`은 서버가 만든 요약문을 달고
**바로** 응답한다(계산은 0.5초). AI 문장은 화면이 뜬 뒤 브라우저가 `/api/cycle/ask`에
`mode: "summary"`로 따로 받아 와서 자리를 바꿔 끼운다. 그래서 모델이 굼떠도 리포트가
늦게 뜨지 않는다 — 예전에는 이 한 번의 호출 때문에 이미 끝난 계산이 20초씩 기다렸다.

그 자동 요약은 **모델당 7초**만 준다(`attemptCapMs`). 사용자가 기다리는 화면이 아니니
최신 모델을 먼저 찔러 보고 굼뜨면 즉시 다음 후보로 넘긴다. 반대로 사용자가 직접 누른
질문은 기다릴 가치가 있으므로 적응형 예산(최대 14초)을 그대로 쓴다.

> 폴백 체인은 **실패했을 때만** 다음 모델로 간다 — 느린 건 실패가 아니다. 그래서 "최신
> 모델을 맨 앞에 두면 느릴 때 알아서 구형으로 넘어가겠지"는 성립하지 않는다. 오히려
> 후보가 늘수록 총 대기 시간만 길어진다. '느리면 넘어가기'는 위의 모델당 상한이 만든다.
