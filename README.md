...
| 신호 | 조건 | 뜻 |
|---|---|---|
| 거래량 폭발 | 거래량 20일 평균 대비 2.0배 이상 | 평소보다 거래량이 확 터진 날 |
| 물량 흡수 | 거래량 1.5배 이상, 종가 변동 ±3.0% 이내 | 거래량은 늘었는데 주가는 거의 안 움직인 날 |
| 고가 마감 | 거래량 1.3배 이상, 당일 고저폭 상위 30%에서 마감 | 거래량 늘면서 고가 근처에서 끝난 날 |
| 누적 매집 | 상승일/하락일 거래량 비율 1.2 이상, OBV 기울기 0.1 이상 | 오르는 날에 거래량이 몰리는 구간 |
| 급등 직전 | 거래량 1.5배 이상 + 이후 20거래일 내 +15% 상승 | 미래 데이터를 쓰는 **과거 검증 전용** |

임계값은 "가끔은 실제로 걸리는" 수준으로 잡는다. 조건을 겹치고 조일수록 매칭이 0에
수렴해 통계도 알림도 무의미해지므로, 각 신호는 **조건 2개 이하**로 유지한다.
`scripts/selftest.ts`가 무작위 워크 픽스처에서 각 신호가 최소 2%의 날에 걸리는지
검사한다 — 임계값이 다시 조여지면 테스트가 깨진다.
