...
lib/
  ...
  indicators.ts             파생 지표
  timeframe.ts              일봉 → 주봉·월봉 집계 (차트 전용)
...
차트 위의 **일봉 / 주봉 / 월봉** 버튼으로 봉을 바꿔 볼 수 있다. 시세 소스가 전부 일봉만 주므로
주봉·월봉은 받아온 일봉을 `lib/timeframe.ts`가 직접 묶어서 만든다 — 시가는 구간 첫 봉, 종가는
마지막 봉, 고저는 구간 전체 최대·최소, 거래량은 합, 펀딩비는 구간 평균이고 주봉은 월요일에
시작한다. **분석·통계 자체는 계속 일봉 기준이며**, 매칭 화살표는 그 날이 속한 봉에 찍힌다.
