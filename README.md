...
lib/
  ...
  cycle/signals.ts          지표 배터리 (31개 + 코인 펀딩비 3개, 상태 기반)
...
[1] 상승장 시작 라벨링 (lib/cycle/regime.ts)
    ...
    기본값은 코인 -40%/+80%, 주식 -20%/+40%. 화면에서 조절 가능.
    (-20% 뒤 +25% 회복은 상승장 전환이 아니라 조정 되돌림인 경우가 대부분이라 반등 기준을 높게 잡는다)
   ↓
[2] 지표 배터리 (lib/cycle/signals.ts) — 31개 (+ 코인 펀딩비 3개)
    이동평균(50/120/200/365일, 20/60·50/200 골든크로스, 200일선 기울기),
    주봉 30주선·50주선·200주선·12개월선, MACD(일/일 0선/주/월), RSI(일/주/월/과매도탈출),
...
캔들 차트 위에 **한 번에 지표 하나만** 그린다. 30개 넘게 겹치면 아무것도 안 보이기 때문이다.
...
- **A등급만.** 성적표의 지표 31개와 조합 수십 개 중 관문을 다 통과한 것만 보낸다.
...
- **종목 최대 5개.** 한 종목마다 20년치 일봉 + 지표 31개 + 조합 수십 개를 채점하므로
