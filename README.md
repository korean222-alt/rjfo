...
| 명령 | 조건 | 해석 |
|---|---|---|
| 거래량 폭발 | 20일 평균 거래량 대비 3.0배 이상 | 기존 명령을 그대로 유지 |
| 물량 흡수 | 20일 평균 대비 2.0배 이상, 종가 변동 ±2.0% 이내 | 거래량은 급증했지만 가격 변동은 제한된 날 |
| 고가 마감 | 20일 평균 대비 1.8배 이상, 당일 고저폭 상위 25%에서 마감 | 기존 고가마감형 조건을 그대로 유지 |
| 급등 직전 | 이후 20거래일 내 +20% 상승 여부 | 미래 데이터를 쓰는 과거 검증 전용 조건 |
| 누적 매집 | 최근 20일 상승일 거래량합 ÷ 하락일 거래량합 1.5배 이상, OBV 기울기 0.3 이상 | 기존 누적형 조건을 그대로 유지 |
| 상승 전 압축 | ATR 비율 0.8 이하, 20일 평균 대비 거래량 1.2배 이상, 종가 위치 0.6 이상 | 변동폭이 줄어든 상태의 선행 관찰 신호 |
| 강한 돌파 | 20일 평균 대비 거래량 2.0배 이상, +2.0% 이상, 종가 위치 0.85 이상 | 가격과 거래량이 동시에 확장된 신규 신호 |
| 거래량 확장 | 20일 평균 대비 거래량 2.5배 이상, 60일 z-score 1.5 이상 | 방향성을 판단하기 전 이례적 거래량부터 선별 |
| 수급 개선 | 최근 20일 상승·하락일 거래량 비율 1.4 이상, OBV 기울기 0.15 이상 | 단기 수급 개선을 별도로 관찰 |
