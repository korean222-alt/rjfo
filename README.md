...
| 명령 | 조건 | 해석 |
|---|---|---|
| 매집 신호 | 거래량 증가, 작은 가격 변동, 상승일 수급 우위, OBV 개선 | "세력 매집", "누적 매집", "물량 흡수"처럼 겹치던 표현을 하나의 조건으로 통합 |
| 상승 전 압축 | ATR 비율 0.8 이하, 거래량 개선, 고가 쪽 종가 | 변동폭이 줄어든 상태에서 매수 압력이 붙는지 확인하는 선행 관찰 신호 |
| 초기 돌파 | 거래량 증가, +1% 이상 상승 마감, 고가 부근 종가 | 당일 매수 우위가 분명한 초반 돌파 시도 확인 |
| 강한 돌파 | 거래량 2배 이상, +2% 이상 상승 마감, 고가권 종가 | 가격과 거래량이 동시에 확장된 강한 당일 돌파 확인 |
| 거래량 확장 | 거래량 2.5배 이상, 60일 기준 z-score 1.5 이상 | 방향성을 판단하기 전 이례적 거래량부터 선별 |
| 수급 개선 | 상승일/하락일 거래량 비율 1.4 이상, OBV 기울기 0.15 이상 | 단기 수급이 개선되는지 별도로 관찰 |
| 과거 급등 전 검증 | 이후 20거래일 내 +20% 상승 여부 | 미래 데이터를 쓰는 백테스트 전용 조건이며 실시간 신호가 아님 |
