...
lib/
  ...
  indicators.ts             파생 지표
  filter.ts                 FilterSpec 적용 + 국면 묶기
  rarity.ts                 희귀도 (조건 지표의 전체 분포 내 백분위)
  ma-cross.ts               이동평균선 + 골든/데드크로스 탐지
  stats.ts                  forward return, base rate, edge
  presets.ts                프리셋 정의
  gemini.ts                 Gemini 호출 + 모델 폴백 체인 + 시간 예산
  validate-spec.ts          LLM 출력 검증 (알려진 값만 통과)
components/                 TickerInput, CommandInput, SummaryCard, VolumeChart, MatchList, MaControls
...
"한 국면 = 신호 수십 개"가 된다. 그래서 조건 강화와 별개로 아래 신호 정리를 함께 건다.
...
조건을 만족한 날을 전부 신호로 세지 않는다. **시간 간격으로는 절대 솎아내지 않는다** —
"직전 신호 이후 N거래일 대기" 방식은 정작 그 구간에서 제일 중요한 날을 통째로 잘라낸다.
대신 "얼마나 드문 날인가"로 정리한다.

### 희귀도

각 조건 지표가 그 종목 **전체 분포에서 어느 위치인지**를 백분위로 매긴 값(0~100).

- `>=` / `>` 조건: 값이 클수록 드물다 → 자기 이하인 값의 비율
- `<=` / `<` 조건: 값이 작을수록 드물다 → 자기 이상인 값의 비율
- `AND`는 조건별 백분위의 평균, `OR`은 최댓값 (스펙의 `logic`이 뜻하는 바를 그대로 따른다)
...
| 필드 | 뜻 |
|---|---|
| `cluster_gap` | 이만큼 이내로 붙은 매칭일을 한 국면으로 묶는다 (1 = 연속일만, 상한 20) |
| `cluster_pick` | 국면 대표일: `"rarest"`(기본) = 가장 희귀한 날, `"first"` = 가장 이른 날 |
| `top_pct` | 그렇게 남은 신호 중 희귀도 상위 몇 %만 남길지 (1~100, 100 = 전부) |

적용 순서: **조건 필터 → `lookahead` → 국면 묶기 → 희귀도 순위 컷**

핵심은 국면 대표일을 **가장 희귀한 날**로 고른다는 점이다. 국면 한가운데에 제일 강한 날이
있어도 그 날이 살아남는다. `top_pct`도 절대 임계값이 아니라 순위로 자르므로,
조건이 이미 빡빡해 매칭이 3일뿐인 프리셋도 신호가 0개가 되지 않는다
(절대 임계값을 쓰면 가드 성격의 조건이 섞인 프리셋만 평균이 통째로 내려가 전멸한다).

셋 다 스펙에 없으면 프리셋 기본값(`PRESET_SIGNAL_RULES`)을 쓰고, 프리셋도 없으면 정리 없이
동작한다. 결과 화면 **신호 정리** 카드에서 직접 바꿔 즉시 다시 계산할 수 있고,
`rawMatchCount`(조건 충족일) → `stats.matchCount`(최종 신호)가 함께 표시된다.

## 이동평균선 · 골든/데드크로스

결과 화면의 **이동평균선** 카드에서 단기·장기 기간을 직접 정하면(2~400일, 프리셋 5/20 ·
20/60 · 50/200), 차트에 두 선이 그려지고 교차한 날이 표시된다.

- 캔들 **위** — <span>▲GC</span> 골든크로스(단기선이 장기선을 위로 돌파),
  <span>▼DC</span> 데드크로스(아래로 이탈)
- 캔들 **아래** — 파란 화살표, 현재 조건에 매칭된 신호 날짜

두 선이 정확히 겹치기만 하고 되돌아간 봉은 교차로 세지 않는다. 다음 봉에서 실제로 뚫고
지나가야 신호다 — 맞닿은 봉을 세면 교차 수가 두 배가 된다.

교차 목록에는 각 교차일 **이후 20거래일 수익률**이 함께 나온다. 계산은 `lib/ma-cross.ts`에
있고 selftest가 검산하며, 이미 받아둔 시계열로 브라우저에서 바로 돌기 때문에 기간을 바꿔도
서버를 다시 부르지 않는다. 선택한 기간은 브라우저에 저장된다.
...
| 물량 흡수 | 20일 평균 대비 2.0배 이상, 종가 변동 ±2.0% 이내 | 연속일만 묶음 |
| 고가 마감 | 20일 평균 대비 1.8배 이상, 당일 고저폭 상위 25%에서 마감 | 연속일만 묶음 |
| 급등 직전 | 이후 20거래일 내 +20% 상승 여부 | — (미래 데이터를 쓰는 과거 검증 전용) |
| 누적 매집 | 상승·하락일 거래량 비율 **2.0** 이상, OBV 20일 기울기 **0.6** 이상, `vol_ma_ratio_20_50` 1.1 이상, 20일선 대비 +12% 이내 | **상위 30% · gap 5일** |
| 조용한 매집 | ATR 비율 0.85 이하, OBV 20일 0.4 / 60일 0.1 이상, 종가 변동 ±3% 이내, 20일선 대비 +8% 이내 | **상위 30% · gap 5일** |
| 상승 전 압축 | ATR 비율 0.8 이하, 거래량 1.2배 이상, 당일 고저폭 상위 40% 마감 | 상위 50% · gap 3일 |
| 거래량 소진 | `vol_ma_ratio_20_50` 0.8 이하, 거래량 0.9배 이하, 60일 고점 대비 -20% 이내 | 상위 50% · gap 5일 |
| 강한 돌파 | 거래량 2.0배 이상, 종가 +2.0% 이상, 당일 고저폭 상위 15% 마감 | 연속일만 묶음 |
| 박스 돌파 | 종가가 직전 60일 최고가 돌파, 거래량 2.0배 이상, 고저폭 상위 30% 마감 | gap 2일 |
| 거래량 확장 | 거래량 2.5배 이상, 60일 거래량 z-score 1.5 이상 | 연속일만 묶음 |
| 눌림목 지지 | 20일선 대비 -8%~+3%, 거래량 0.9배 이하, 고저폭 상위 40% 마감, 상승·하락일 거래량 비율 1.2 이상 | 상위 50% · gap 3일 |
| 수급 개선 | 상승·하락일 거래량 비율 **1.6** 이상, OBV 20일 기울기 **0.3** 이상, 20일 중 상승일 비율 50% 이상 | **상위 30% · gap 5일** |

굵게 표시한 값이 이번에 바뀐 부분이다. **누적 매집**과 **수급 개선**은 전부 상태 지표로만
이뤄져 있어 예전 임계값(1.5 / 0.3, 정리 없음)으로는 한 국면이 신호 수십 개로 부풀려졌다.
임계값을 올리고, 방향이 같은 조건(거래량 베이스 상승·상승일 비율)을 더하고,
정리 규칙을 함께 걸어 "국면 하나 = 신호 한두 개"가 되게 했다.

이벤트 지표 기반 프리셋은 원래 드물게 뜨므로 순위 컷을 걸지 않는다 — 5년 기준 연 5회 안팎이다.
